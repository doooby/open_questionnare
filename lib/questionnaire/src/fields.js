...
export class Fields {
    ...
    getOptionText (field_name, value, lang, translate) {
        const field = this.index[field_name];
        return this.options.getTextFor(field, value, lang, translate);
    }
...
function buildControl (field, options) {
    const def = {
        type: options.control || Fields.TYPE_CONTROL[field.type]
    };
...
class OptionsList {
    ...
    getPairs (lang, translate) {
        if (this.items) {
            const items = this.items.map(item => item.optionKey());
